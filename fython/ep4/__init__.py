from fython.ep4.string_with_arrows import string_with_arrows
import string
...
LETTERS_DIGITS = LETTERS + DIGISTS
...
class Lexer:
    ...
    def make_mul_or_power(self):
        ...
        self.advance()
        if self.current_char == '*':
            result = Token(TT_POW, pos_start=pos_start, pos_end=self.pos)
            self.advance()
            return result
        else:
