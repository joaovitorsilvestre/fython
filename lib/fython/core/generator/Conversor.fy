        ...
        |> Elixir.Enum.map(
            lambda elements_or_unpack:
                case elements_or_unpack:
                    ...
                    _ -> Elixir.Enum.map(elements_or_unpack, lambda i: config)
        )
        |> Elixir.Enum.reduce(
            lambda item, acc:
                (
...
def convert_binop((:binop, meta, [left, op, right]), config):
    ...
    (elixir_op[op], meta, [convert(left, config), convert(right, config)])
...
def convert_def_or_defp((node_type, meta, [name, args, guards, statements]), config) if node_type in [:def, :defp]:
    args = Elixir.Enum.map(args, lambda i: config)
...
def convert_struct_def_node(
    ...
    functions_struct = Elixir.Enum.map(functions_struct, lambda i: config)
    ...
    struct_statements = [
        (
            ...
            [Elixir.Enum.map(
                struct_fields,
                lambda (n, d):
                    (Elixir.String.to_atom(n), convert(d, config))
            )]
        ),
        *functions_struct
    ]
...
def convert_struct_module((:statements, meta, body), module_name, config):
    ...
    metadata_statements = Elixir.Enum.map(metadata_statements, lambda i: config)
